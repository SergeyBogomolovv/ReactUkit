...
export default function Attrativeness() {
  const [attractive, setAttractive] = useState([
    {src: './pngs/attrativnes/glusi.jpg', name: 'Александр Глускер', text:'В общем, Глускер проработал в колледже уже 14 лет, и за это время успел много чего сотворить. Многие студенты его ненавидели, за то что его стиль обучения был слишком жесткий - он давал слишком много материала, но эти знания не всегда были нужны на практике. Но самое шокирующее, это то что он приставал к студентам(не студенткам!). Теперь он увольняется, под давлением студентов, Антонины Петуховой и Ирины Дзюбы. Все таки забавный был препод, но если серьезно, это очень мерзко. Если что заявление об увольнении он написал 12.11.2023, на своей страничке вк он подписал это как "пересмотрение планов на жизнь". В заключение, можно сказать что это очень противно что такой человек проработал так много времени и мог все это время скрываться, чего не скажешь о следующем преподе'},
    {src: './pngs/teachers/valeev.png', name: 'Валеев Михаил', text:'У него похожая ситуация с Глускером - он тоже педофил-гей, в личных разговорах со студентами он звал их пить вместе и сразу давал понять какой он ориентации.По его внешности впринципе можно внешне догадаться кто он, поэтому он и проработал всего год, но ушел тоже по "собственному желанию"'},
    {src: './pngs/attrativnes/novikov.png', name: 'Новиков Дмитрий', text:'Тоже вероятно препод педофил. Вероятно лишь потому, что это только слухи, однако слухи из воздуха не появляются.'},
    ...
    <>
      {load !== true
              ? <TransitionGroup className={cl.container}>
                  {preLoad.map(card =>
                  <CSSTransition key={card.text} timeout={500} classNames='attractive'>
